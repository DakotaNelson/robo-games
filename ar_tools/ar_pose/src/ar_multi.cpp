...
// DEPRECATED: Fuerte support ends when Hydro is released
  ...
  void ARMultiPublisher::getTransformationCallback (const sensor_msgs::ImageConstPtr & image_msg)
  {
    ...
    for (i = 0; i < objectnum; i++)
    {
      ...
      {
        if (i == 0) {
          cv::circle(capture_->image, cv::Point(marker_info[j].pos[0],marker_info[j].pos[1]), 5, cv::Scalar(0,0,255));
          for (int k = 0; k < 4; k++) {
            cv::circle(capture_->image, cv::Point(marker_info[j].vertex[k][0],marker_info[j].vertex[k][1]), 5, cv::Scalar(255,0,0));
          }
        }
